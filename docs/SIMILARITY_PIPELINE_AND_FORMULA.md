...
> 아래 §1~§5는 2025-02-23의 비교용 스냅샷이다. 2026-08-08부터 적용되는 최신 운영 모델은 다음 §0을 기준으로 한다.

---

## 0. 최신 운영 모델: movement-v3 (2026-08-08)

- **1단계 후보 축소**: 요청 반경/기간으로 먼저 필터링하고, 시간에 따라 넓어지는 이동 가능 반경과 종·크기·색상 토큰·태그 규칙 점수로 상위 50~100건만 남긴다.
- **이동 가능 반경**: `min(request_radius_km, 0.45 + 0.65 * sqrt(유실 후 시간 + 1))` km. 시간에 따라 완만하게 넓어지며 요청 반경을 넘지 않는다.
- **이동 점수**: 위 반경의 절반을 표준편차로 한 Gaussian 감쇠다. 따라서 오래 지났다고 먼 제보가 자동으로 유리해지지 않고, 해당 시점의 가능한 반경을 크게 벗어나면 빠르게 낮아진다.
- **2단계 재랭킹**: 색상·무늬 전용 임베딩만 사용한다. 종과 크기는 선택형 값의 규칙 점수, 특이사항은 최대 10점의 보너스이며 메모는 추천 계산에서 제외한다.
- **최종 점수**: 이동 가능성 40점, 종 15점, 크기 10점, 색상·무늬 25점, 특이사항 보너스 10점. 각 기여도는 추천 API의 `scoreBreakdown`으로 반환되어 카드 막대에 그대로 표시된다.
- **색상 의미 점수**: `0.70 * 색상 임베딩 유사도 + 0.30 * 색상 토큰 일치도`. 색상 임베딩 프롬프트에는 색·무늬 표현만 넣는다.
- **관련 코드**: `supabase/migrations/20260808020000_recommendation_movement_and_explanations.sql`, `src/shared/lib/embedding.ts`, `src/features/recommendations/components/RecommendationCard.tsx`.
