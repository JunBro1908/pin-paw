...
test("owner lost-post paths restore precise trail coordinates", async () => {
  ...
  assert.doesNotMatch(
    sql,
    /floor\(st_y\(s\.location::geometry\)\s*\/\s*0\.05\)/
  );
  assert.match(
    sql,
    /grant execute[\s\S]*get_my_lost_post_paths[\s\S]*to authenticated/i
  );
});
...
test("detail sheet and card constrain desktop width", async () => {
  const [sheet, card] = await Promise.all([
    readFile(
      "src/features/sightings/components/SightingDetailSheet.tsx",
      "utf8"
    ),
    readFile(
