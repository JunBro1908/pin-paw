-- Auth ALL layer cluster policy:
-- zoom ≥15 ≈ individual pins; mid/low zoom clusters only ordinary sightings.
-- Owner sightings + claimed/bookmark endpoints always emit as precise points
-- (lost-post pins + trails are client overlays outside this RPC).
...
begin
  ...
  -- Match map-domain getGridSize(authenticated=true): ≥15 near-individual.
  if p_zoom_level >= 15 then
    v_grid_size := 0.001;
  ...
  claimed_ids as (
    select distinct claim.sighting_id as id
    from public.lost_post_sighting_claims claim
    join public.lost_posts lp on lp.id = claim.lost_post_id
    where lp.owner_id = v_user_id
      and lp.archived_at is null
  ),
  -- Owner pins + bookmark endpoints stay out of ordinary clusters.
  privileged_points as (
    select vs.*
    from visible_sightings vs
    where vs.user_id = v_user_id
       or exists (select 1 from claimed_ids c where c.id = vs.id)
  ),
  other_points as (
    select vs.*
    from visible_sightings vs
    where not exists (
      select 1 from privileged_points pp where pp.id = vs.id
    )
  ),
  other_clusters as (
    select
