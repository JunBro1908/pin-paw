...
export const scrollablePanelClass = {
  ...
  dropdown:
    "scrollable-dropdown max-h-[min(40vh,16rem)] overflow-y-auto overscroll-contain [-webkit-overflow-scrolling:touch]",
  /** details/summary bodies, nested option grids */
  panel:
    "scrollable-panel max-h-[min(60vh,20rem)] overflow-y-auto overscroll-contain [-webkit-overflow-scrolling:touch]",
  /** Inline lists under accordion sections (e.g. /my 내 제보) */
  list:
    "scrollable-list max-h-[min(50vh,18rem)] overflow-y-auto overscroll-contain [-webkit-overflow-scrolling:touch]",
  /** Long result card lists (e.g. /recommend 제보 카드) — header stays outside */
  results:
    "scrollable-panel max-h-[min(60vh,28rem)] overflow-y-auto overscroll-contain [-webkit-overflow-scrolling:touch]",
  /** Fixed/modal sheets above bottom nav + home indicator */
  sheet:
    "scrollable-sheet max-h-[min(calc(85vh-var(--bottom-nav-height)-env(safe-area-inset-bottom,0px)),36rem)] overflow-y-auto overscroll-contain [-webkit-overflow-scrolling:touch] pb-[max(0.75rem,env(safe-area-inset-bottom))]",
} as const;
