...
/**
 * Map chrome: brand title (top-left) + legend (bottom-left).
...
export function MapLegend() {
  return (
    <>
      <section
        aria-label="지도 제목"
        className="border-border-subtle bg-surface pointer-events-none absolute top-3 left-3 z-10 max-w-[min(calc(100%-5.5rem),14rem)] rounded-2xl border px-3 py-2 shadow-sm"
      >
        <div className="flex items-center gap-1.5">
          <Icon name="paw" size={18} className="text-status-lost" />
          <p className="text-text-main text-sm font-semibold tracking-tight">
            PinPaw 지도
          </p>
        </div>
        <p className="text-text-caption mt-1 text-xs leading-snug">
          반려동물의 흔적을 찾아보세요.
        </p>
      </section>
